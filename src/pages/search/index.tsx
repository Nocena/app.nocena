...
const SearchView = () => {
  ...
  const convertToSearchUser = useCallback((authUser: any): SearchUser => {
    return {
      id: authUser.id,
      ...
      followers: Array.isArray(authUser.followers) 
        ? authUser.followers.map((f: any) => typeof f === 'string' ? f : f.id)
        : [],
      // Convert following array to array of IDs  
      following: Array.isArray(authUser.following)
        ? authUser.following.map((f: any) => typeof f === 'string' ? f : f.id)
        : []
    };
  }, []);
  ...
  const fetchUserData = useCallback(async (showLoading = true) => {
    ...
    try {
      ...
      localStorage.setItem('nocena_cached_users', JSON.stringify({
        data: searchUsers,
        timestamp: Date.now()
      }));
    } catch (error) {
    ...
    const updateUserFollowState = (userList: SearchUser[]) => {
      return userList.map((u) => {
        if (u.id === targetUserId) {
          const isCurrentlyFollowing = u.followers && Array.isArray(u.followers) && 
  ...
  const handleProfileRedirect = useCallback((clickedUser: SearchUser) => {
  ...
  const userList = useMemo(() => {
    ...
    return filteredUsers.map((userData) => {
      const isFollowing = userData.followers && Array.isArray(userData.followers) && 
