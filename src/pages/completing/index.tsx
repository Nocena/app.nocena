...
const CompletingView = () => {
  ...
  const handleSubmitChallenge = useCallback(async () => {
    ...
    try {
      ...
      const mediaMetadata = await uploadMediaToIPFS(
        videoBase64,
        selfieBase64,
        tempChallengeId,
        user.id
      ...
      if (isAIChallenge && frequency) {
        ...
        if (frequency === 'daily') {
          ...
          dailyString = 
            dailyString.substring(0, dayOfYear) + 
            '1' + 
            dailyString.substring(dayOfYear + 1);
          ...
          weeklyString = 
            weeklyString.substring(0, weekOfYear) + 
            '1' + 
            weeklyString.substring(weekOfYear + 1);
          ...
          monthlyString = 
            monthlyString.substring(0, month) + 
            '1' + 
            monthlyString.substring(month + 1);
