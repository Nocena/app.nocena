...
const CompletionItem: React.FC<CompletionItemProps> = ({ profile, completion, isSelf }) => {
  ...
  if (completion.media) {
    ...
    media = typeof completion.media === 'string' ? parseMediaMetadata(completion.media) : completion.media;
  }
    ...
    <div className="bg-[#2A3B4D] rounded-xl p-4 border border-[#4A5B6D]">
      ...
      <div className="flex items-center mb-3">
        ...
        <div className="ml-3">
          ...
          <p className="text-xs text-gray-400">{completionDate.toLocaleTimeString()}</p>
        </div>
      </div>

      {/* Video and selfie content using the resilient loader */}
      {videoUrl || selfieUrl ? (
        <IPFSMediaLoader videoUrl={videoUrl} selfieUrl={selfieUrl} className="mb-3" />
      ) : (
