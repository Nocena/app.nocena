...
import OtherProfileView from '@views/profile/[walletAddress]';
import CompletingView from '@views/completing';
...
const AppLayout: React.FC<{ user: any; handleLogout: () => void }> = ({ user, handleLogout }) => {
  ...
  return (
    <div className="app-container bg-nocenaBg min-h-screen w-full text-white flex flex-col">
      ...
      <main className="flex-grow mt-12 pt-3">
        {router.pathname === '/home' && <HomeView />}
        {router.pathname === '/map' && <MapView />}
        {router.pathname === '/challenges' && <ChallengesView />}
        {router.pathname === '/search' && <SearchView />}
        {router.pathname === '/profile' && <ProfileView />}
        {router.pathname === '/completing' && <CompletingView />}
        {router.pathname.startsWith('/profile/') && (
          <OtherProfileView walletAddress={router.query.walletAddress as string} />
        )}
        {router.pathname === '/createchallenge' && <CreateChallengeView />}
      </main>
